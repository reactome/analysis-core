...
import org.reactome.server.analysis.core.model.EntitiesContainer;
...
public class InteractorsBuilder {
    ...
    public void build(EntitiesContainer entities, InteractorsDatabase interactorsDatabase) {
        ...
        Set<EntityNode> allNodes = entities.getAllNodes();
        int i = 0, tot = allNodes.size();
        for (EntityNode entityNode : allNodes) {
            if (Main.VERBOSE && ++i % 25 == 0) {
                System.out.print("\rRetrieving interactors >> " +  i + "/" + tot);
            }
            if (!entityNode.getModifications().isEmpty()) continue;
            //TODO: if(!entityNode.isDirectlyInADiagram()) continue;
